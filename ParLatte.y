...
Program :: { Program }
Program : ListStmt { AbsLatte.Prg (reverse $1) }
Block :: { Block }
Block : '{' ListStmt '}' { AbsLatte.Blck (reverse $2) }
ListStmt :: { [Stmt] }
...
Stmt : ';' { AbsLatte.Empty }
     ...
     | Type Ident '(' ListArg ')' Block { AbsLatte.FDecl $1 $2 $4 $6 }
     | Ident '=' Expr ';' { AbsLatte.Ass $1 $3 }
     | Ident '++' ';' { AbsLatte.Incr $1 }
     | Ident '--' ';' { AbsLatte.Decr $1 }
     | 'return' Expr ';' { AbsLatte.Ret $2 }
     | 'return' ';' { AbsLatte.VRet }
     | 'if' '(' Expr ')' Block { AbsLatte.Cond $3 $5 }
     | 'if' '(' Expr ')' Block 'else' Block { AbsLatte.CondElse $3 $5 $7 }
     | 'while' '(' Expr ')' Block { AbsLatte.While $3 $5 }
     | Expr ';' { AbsLatte.SExp $1 }
     | 'print' Expr ';' { AbsLatte.Print $2 }
Arg :: { Arg }
Arg : Type Ident { AbsLatte.ArgVal $1 $2 }
    | Type '&' Ident { AbsLatte.ArgRef $1 $3 }
ListArg :: { [Arg] }
ListArg : {- empty -} { [] }
        | Arg { (:[]) $1 }
        | Arg ',' ListArg { (:) $1 $3 }
...
Type :: { Type }
Type : PType { AbsLatte.ArgTypeP $1 }
     | VType { AbsLatte.VoidType $1 }
PType :: { PType }
PType : 'int' { AbsLatte.Integer }
      | 'string' { AbsLatte.String }
      | 'bool' { AbsLatte.Bool }
VType :: { VType }
VType : 'void' { AbsLatte.Void }
ListType :: { [Type] }
ListType : {- empty -} { [] }
         | Type { (:[]) $1 }
         | Type ',' ListType { (:) $1 $3 }
